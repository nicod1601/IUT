		...
		try
		{
			...
			while ( scDept.hasNextLine() )
			{
				...
				scDept.nextLine();
			        ...
			        scDept = new Scanner ( new FileInputStream ("departement.data") );
		...
		try
		{
			...
			while ( scReg.hasNextLine() )
			{
				...
				scReg.nextLine();
			        ...
			        scReg = new Scanner ( new FileInputStream ("region.data") );
			
			for (int cpt = 0; cpt < nbLigne; cpt++)
			{
				enreg = scReg.nextLine();
				dec   = new Decomposeur ( enreg );
				tab[cpt] = new Region (dec.getInt(0), dec.getString(1), dec.getInt(2));
