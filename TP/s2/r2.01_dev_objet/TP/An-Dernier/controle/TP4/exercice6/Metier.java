...
public class Metier
{
	...
	public Metier ()
	{
		tabDept   = new Departement[101];
		tabRegion = new Region     [ 18];
	...
	public String[] listerRegion()
	{
		...
		for (int cpt=0; cpt < this.nbRegion ; cpt++ )
			liste[cpt] = tabRegion[cpt].getNom();
	...
	private void chargerDept()
	{
		...
		try
		{
			Scanner scDept = new Scanner ( new File ( "departement.data" ), "UTF-8" );
			...
			while ( scDept.hasNextLine() )
			{
				...
				tabDept[indiceDept++] = new Departement ( numDept, nomDept, populationDept, superficieDept );
			}

			this.nbDept = indiceDept;
		}
	...
	private void chargerRegion()
	{
		...
		try
		{
			...
			while ( scRegion.hasNextLine() )
			{
				...
				tabRegion[indiceRegion++] = new Region ( numRegion, nomRegion, nbDept );
			}

			this.nbRegion = indiceRegion;
		}
		catch(Exception e){ e.printStackTrace(); }
	}
	...
	private Departement rechercherDept ( String numero)
	{
		for ( int cpt=0; cpt < this.nbDept ; cpt ++ )
			if ( tabDept[cpt] != null && tabDept[cpt].getNumero().equals(numero) )
				return tabDept[cpt];
