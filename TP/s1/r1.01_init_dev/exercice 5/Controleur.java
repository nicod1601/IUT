...
public class Controleur extends Controle
{
	...
	public void jouer (String touche)
	{
		...
		// Ajouter la logique pour le saut
		if ( touche.equals ( "CR-" ) ) {
			this.metier.sauter( 'N' ); // Saut en haut
		}
		if ( touche.equals ( "CR-FL-G" ) ) {
			this.metier.sauter( 'O' ); // Saut à gauche
		}
		if ( touche.equals ( "CR-FL-B" ) ) {
			this.metier.sauter( 'S' ); // Saut en bas
		}
		if ( touche.equals ( "CR-FL-D" ) ) {
			this.metier.sauter( 'E' ); // Saut à droite
		}

		// Vérifier si le personnage est dans le vide
